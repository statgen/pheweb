    ...
    stage('Deploy') {
	    steps {
                withCredentials([file(credentialsId: 'jenkins-sa', variable: 'gcp')]) {
                    ...
                    sh '''if helm ls | grep bstaging > /dev/null  ; then COMMAND=upgrade ; else  COMMAND=install ; fi ; helm ${COMMAND} ${SUBDOMAIN} ./deploy/pheweb --set pheweb.subdomain=${SUBDOMAIN} --set pheweb.mount=${MOUNT}'''
		}
	    }
	}
    }
