    ...
    stage('Deploy') {
	    steps {
                withCredentials([file(credentialsId: 'jenkins-sa', variable: 'gcp')]) {
                    ...
                    sh '''if helm ls | grep pheweb > /dev/null  ; then  helm upgrade staging-pheweb ./deploy/pheweb -f ./deploy/pheweb/staging-values.yaml --set image.tag=ci-${env.GIT_COMMIT} ; else helm install staging-pheweb ./deploy/pheweb  -f ./deploy/pheweb/staging-values.yaml --set image.tag=ci-${env.GIT_COMMIT} ; fi ; '''
