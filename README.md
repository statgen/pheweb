...
- requires `$data_dir/manhattan/<phenocode>.json`
- requires `$data_dir/qq/<phenocode>.json`
...
- `source_filenames_phenocode_extracting_regex`: see information for `$data_dir/phenos.csv` below.
...
- `use_vanderbilt_phewas_icd9_info_and_categories`: see information for `data/phenos.json` below.
...
- uses `use_vanderbilt_phewas_icd9_info_and_categories` from `config.config`.
...
- format (example [here](https://raw.githubusercontent.com/statgen/pheweb/master/data/phenos.json)):
    ...
    ```
    {
        "<phenocode>": { # <phenocode> must uniquely identify this phenotype and be URL-safe.
            ...
            "phenostring": "<phenostring>" # <phenostring> will be the header on the page.
            # Optionally, may also include the key "icd9_info".  See the example file for the format.
        }, ...
    }
    ```
...
- uses `source_filenames_phenocode_extracting_regex` (a regex) from `config.config`
- generated by `0_0_make_phenos_csv.py`
- format:
    - comma-delimited
    - columns:
        - `phenocode`: the short string that will identify this phenotype. Must be URL-safe.
        - `src_filename`: the path to the input file with data for this phenotype.
        - `category_string`: must be added manually if `use_vanderbilt_phewas_icd9_info_and_categories` (in `config.config`) is False.
...
**`matrix.tsv`:**
- requires `$data_dir/augmented_pheno/<phenocode>`
...
- format:
    ...
    - columns:
        ...
        - `pval-<phenocode>`: like `0.366233` or `1.23e-4`.  There must be one column like this for each phenotype.

**`$data_dir/augmented_pheno/<phenocode>`:**
...
**`$data_dir/manhattan/<phenocode>.json`:**
- requires `$data_dir/augmented_pheno/<phenocode>`
- generated by `3_2_make_manhattan_for_each_pheno.py`

**`$data_dir/qq/<phenocode>.json`**
- requires `$data_dir/augmented_pheno/<phenocode>`
