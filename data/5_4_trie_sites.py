...
sites_rsids_trie = marisa_trie.BytesTrie(lines)
...
rsids_sites_trie = marisa_trie.BytesTrie((rsid.decode('ascii'), chrom_pos_ref_alt.encode('ascii')) for (chrom_pos_ref_alt, rsids) in lines for rsid in rsids.split(','))
