...
TransformationFunctions.set<number,number>("logneglog", function(x : number) {
    ...
    if (pScaled > config.vs_configuration.loglog_threshold) {
        pScaled = config.vs_configuration.loglog_threshold * Math.log10(pScaled) / Math.log10(config.vs_configuration.loglog_threshold)
    }
    return pScaled
})
...
export const init_locus_zoom = (region : Region) : LocusZoomContext =>  {
    ...
    const gwascatSource : Array<number> = application.genome_build === 37 ? [2,3] : [1,4]
    ...
    if (application?.ld_service?.toLowerCase() === 'finngen') {
