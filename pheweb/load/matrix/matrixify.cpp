...
// test with:
// clear&& g++ -std=c++11 -lz matrixify.cpp &&echo --&& ./a.out 4 ~/PROJECTS/pheweb/test/data_dir/sites/sites.tsv '/Users/peter/PROJECTS/pheweb/test/data_dir/augmented_pheno/*' ~/PROJECTS/pheweb/test/data_dir/matrix.tsv.gz &&echo --&& zcat /Users/peter/PROJECTS/pheweb/test/data_dir/matrix.tsv.gz | head
...
public:
    ...
    inline void attach(std::istream* stream_) {
        stream = stream_;
        next();
    }
    inline void next() {
...
int run(char *sites_fname, char *augmented_pheno_glob, char *matrix_fname) {
    static const size_t CHR_COL = 0;
    static const size_t POS_COL = 1;
    static const size_t REF_COL = 2;
    static const size_t ALT_COL = 3;

    BgzipWriter writer(matrix_fname);

    LineReader sites_reader;
    sites_reader.attach(new std::ifstream(sites_fname));
    ...
    std::vector<int> aug_n_per_assoc_fields(N_phenos); // initialized to 0s.
    ...
    for (size_t i=0; i < N_phenos; i++) {
        ...
        // std::cout << "paf: [" << per_assoc_fields << "]" << std::endl;
    ...
    while(1) {
        ...
        std::string debugline = "1\t869334\t";
        bool debug = (0 == sites_reader.line.compare(0, debugline.size(), debugline));
        if (debug) {
            std::cout << "cpra: [" << sites_reader.line.substr(0, pos_after_cpra) << "]" << std::endl;
        }

        for (size_t i=0; i<N_phenos; i++) {
            if (debug)
                std::cout << "comparing cpra [" << aug_readers[i].line.substr(0, pos_after_cpra) << "] for pheno [" << aug_fnames[i] << "] with " << aug_n_per_assoc_fields[i] << " per-assoc fields" << std::endl;
            ...
            if (!aug_readers[i].eof() && 0 == sites_reader.line.compare(0, pos_after_cpra, aug_readers[i].line, 0, pos_after_cpra)) { // CPRAs match.
                ...
                assert(n_fields(aug_readers[i].line) == n_per_variant_fields + aug_n_per_assoc_fields[i]);
                if (debug) {
                    std::string per_assoc_values = aug_readers[i].line.substr(sites_reader.line.size(), std::string::npos);
                    std::cout << "- match! - rest of line: [" << per_assoc_values << "]" << std::endl;
                    writer.write(per_assoc_values); // TODO: avoid intermediate std::string by using aug_readers[i].line.c_str()
                } else {
                ...
                for (size_t j=0; j<aug_n_per_assoc_fields[i]; j++) writer.write("\t."); // TODO remove '.'
            }
        }
        writer.write("\n");
        // if (i_site++ > 10) break;
