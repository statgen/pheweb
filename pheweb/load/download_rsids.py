...
# It downloads resources from <https://resources.pheweb.org/>.  To populate that site, I run `pheweb download-rsids-from-scratch` and `cp generated-by-pheweb/resources/* ...`.
# `pheweb download-rsids-from-scratch` _should_ always work, so it's always a fallback, but it should only happen when explicitly chosen.
