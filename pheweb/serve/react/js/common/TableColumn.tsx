...
const phenotypeColumns = {
    ...
    chipVariant: {
        ...
        Cell: props => (
            <a data-html={true}
               data-tip={ReactDOMServer.renderToString(<img style={{maxWidth: '100%', maxHeight: '100%'}} id='cplot'
    ...
    chipGeneMostSevere: {
        ...
        Cell: props => (
            props.value == 'NA' ? props.value :
                <a href={"http://www.omim.org/search/?index=entry&sort=score+desc%2C+prefix_sort+desc&start=1&limit=10&search=" + props.value}
                   target="_blank">{props.value}</a>
        ),
        width: columnWith(80)
    },
    ...
    pos:
        {
            ...
            accessor: 'pos',
            Cell: props => (
                <a href={`/variant/${props.original.chrom}-${props.original.pos}-${props.original.ref}-${props.original.alt}`}>{props.value}</a>),
            filterMethod: (filter, row) => {
            ...
            Cell: props => props.original.ukbb ?
                <div>{(Number(props.original.ukbb.beta) >= 0) ?
                    <span style={{color: 'green', float: 'left', paddingRight: '5px'}}
                          className="glyphicon glyphicon-triangle-top" aria-hidden="true"></span> :
                    (Number(props.original.ukbb.beta) < 0) ?
                        <span style={{color: 'red', float: 'left', paddingRight: '5px'}}
                              className="glyphicon glyphicon-triangle-bottom" aria-hidden="true"></span> :
