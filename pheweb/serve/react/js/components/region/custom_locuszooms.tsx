...
Data.FG_LDDataSource.prototype.getURL = function(state, chain, fields) {
    ...
    if (this.lz_conf.ld_service.toLowerCase() == 'finngen') {
	var windowSize = Math.min(state.end - state.start + 10000, this.lz_conf.ld_max_window)
	...
	return refvar ? this.url + refvar + "/" + this.lz_conf.ld_ens_pop + "?window_size=" + this.lz_conf.ld_ens_window : this.url + ' lead variant has no rsid, could not get LD'
    }
