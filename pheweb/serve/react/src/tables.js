...
const lofTableCols = [{
    ...
    Header: () => (<span title="gene" style={{textDecoration: 'underline'}}>gene</span>),
    accessor: 'gene',
    Cell: props => (<a href={"/gene/" + props.value}
                       rel="noopener noreferrer"
                       target="_blank">{props.value}</a>),
    minWidth: 80
}, {
...
const chipTableCols = [{
    ...
    Cell: props => (
	props.value === 'NA' ? props.value : <a href={"/gene/" + props.value}
                                          rel="noopener noreferrer"
                                          target="_blank">{props.value}</a>
    )
}, {
    ...
    filterMethod: null,
    Cell: props => {
	const grch37 = props.value && props.value !== 'NA' ? (<a rel="noopener noreferrer"
                                                           href={"https://gnomad.broadinstitute.org/variant/" + props.value.replace(/:/g, '-') + '-' + props.original.variant.split(':').slice(2).join('-')} target="_blank">gn2</a>) : null
	const grch38 = props.original.gnomad3 === 1 ? (<a style={{float: 'right'}}
                                                    rel="noopener noreferrer"
                                                    href={"https://gnomad.broadinstitute.org/variant/" + props.original.variant.split(':').join('-') + '?dataset=gnomad_r3'} target="_blank">gn3</a>) : null
	return <div>
	    {grch37}
	{grch38}
	</div>
...
const codingTableCols = [{
	...
	return (
        ...
        href={`http://${window.release_prev.toLowerCase()}.finngen.fi/variant/${props.original.variant.replace(/:/g, '-')}`}
        rel="noopener noreferrer"
