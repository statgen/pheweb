...
function get_fdr(maf, pval) {
    for (let maf_bin of window.model.fdr_max_maf_pval_bins) {
        if (maf <= maf_bin.max_maf) {
            for (let pval_bin of maf_bin.max_pval_bins) {
...
function create_gwas_plot(variant_bins, unbinned_variants) {
    ...
    if (window.model.fdr_max_maf_pval_bins) {
        for (let v of unbinned_variants) {
    ...
    $(function() {
        ...
        var color_by_chrom = d3.scale.ordinal()
            ...
            .range(['rgb(150,150,150)', 'rgb(100,100,100)']);
        ...
        var color_by_fdr = d3.scale.linear()
            .domain([0, 1])
            .range(['blue', 'red']);

        function get_color_for_variant(v) {
            if (v.fdr) {
                return color_by_fdr(v.fdr);
            }
            return color_by_chrom(v.chrom);
        }
        ...
        gwas_svg.selectAll('text.chrom_label')
            ...
            .style('fill', get_color_for_variant);
        ...
        var bins = gwas_plot.append('g')
            ...
            .each(function(d) { //todo: do this in a forEach
                ...
                d.color = get_color_for_variant(d);
            });
